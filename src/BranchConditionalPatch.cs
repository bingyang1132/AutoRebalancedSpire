...
///     之间随机）；改版把组装打击整个删了，分支直接通向组装或瓦解，机器里没有 <c>RAND</c>。
///   </item>
///   <item>
...
internal static class BranchConditionalPatch
{
        ...
        ref string __result)
    {
        ...
        // 同族信徒：改版给它加了一条条件分支，但**忘了把分支本身加进状态列表**。
    ...
    /// <summary>灵魂枢纽身上的「灵魂凋亡」有没有记满。</summary>
    /// <remarks>
    /// 灵魂枢纽三条分支问的都是同一件事：它对某个玩家的强化攻击打中次数有没有到 12。
    /// 到了就改成「灵魂标记」（收掉灵魂凋亡、给玩家 99 层易伤），接着是「灵魂灼烧」。
    ///
