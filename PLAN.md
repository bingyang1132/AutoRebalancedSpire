...
第三轮把上一轮列的三条都做掉了（组装师的补造、流星锤的滚雪球、求解器原版下也不支持的
那批怪物招式），剩下的只有**新 Boss 门匠**一只。见下面「玩家须知」和「边界」。
...
`tools/run-rebalanced-matrix.ps1` **十一条，2026-09-12 20:12 全部通过**（求解器 `06D43102`，
适配层当日构建 `2f03fa91`）。
